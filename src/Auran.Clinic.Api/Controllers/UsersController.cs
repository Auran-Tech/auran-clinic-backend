...
{
    [Authorize(Policy = PermissionPolicy.Prefix + Permissions.UsersManageStatus)]
    [HttpPut("status")]
    [SwaggerOperation(
        Summary = "Enable or disable a clinic user account",
        Description = "Changes account status inside the authenticated clinic. Normal managers cannot disable another Super User. Disabling an account revokes its refresh tokens and invalidates existing JWT requests immediately.",
    ...
    {
        var result = await userAccountService.SetStatusAsync(
            request.UserId,
            request.IsActive,
            cancellationToken);
    ...
    [SwaggerOperation(
        ...
        Description = "Allows the authenticated user to disable their own account. Refresh tokens are revoked and subsequent JWT requests are rejected until another authorized account re-enables the user.",
