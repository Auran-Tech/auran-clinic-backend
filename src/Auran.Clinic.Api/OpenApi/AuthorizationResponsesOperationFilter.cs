...
public sealed class AuthorizationResponsesOperationFilter : IOperationFilter
{
    public void Apply(OpenApiOperation operation, OperationFilterContext context)
    {
        ...
        {
            Description = "Unauthorized - authentication is missing, invalid, expired, or the session was revoked.",
            Content = content
        });
        operation.Responses.TryAdd("403", new OpenApiResponse
        {
            Description = "Forbidden - the authenticated user does not satisfy the required permission policy.",
            Content = content
        });
    }
}
